...
export function validateRegexPattern(pattern: string): RegexValidation {
  ...
  if (/\\[1-9]/.test(pattern)) {
    return { valid: false, message: 'Backreferences are not supported.' };
  }
  if (
    /\((?:[^()]|\\.)*[+*](?:[^()]|\\.)*\)\s*(?:[+*]|\{)/.test(pattern) ||
    /\.\*.*\.\*/.test(pattern)
  ) {
    return { valid: false, message: 'Avoid nested or repeated wildcard quantifiers.' };
  }
  try {
    new RegExp(pattern, 'i');
    return { valid: true };
  } catch {
    return { valid: false, message: 'This regular expression is not valid.' };
  }
}
...
export function ruleMatches(rule: RoutingRule, record: PageRecord): boolean {
  ...
  return new RegExp(rule.matcher.pattern, 'i').test(record.url.slice(0, MAX_MATCH_URL_LENGTH));
}
